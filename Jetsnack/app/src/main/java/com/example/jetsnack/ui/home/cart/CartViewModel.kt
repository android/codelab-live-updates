...
class CartViewModel(
    ...
    fun checkoutSnacks() {
        ...
        val orderString = StringBuilder( "Placed order for: ")
        val orderLines = SnackRepo.getCart()
        for (i in orderLines.indices) {
            val item = orderLines[i]
            val isLast = i == orderLines.size - 1
            orderString.append(item.snack.name)
            if (!isLast) {
                orderString.append(",")
            }
        }
